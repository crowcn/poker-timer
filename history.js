...
function renderLeaderboard(board) {
  ...
  const data = players.map(player => {
    const rows = records.filter(row => row.phoneLastFour === player.phoneLastFour);
...
function searchPlayerHistory(event) {
  ...
  const player = (window.historyPlayers || []).find(item => item.phoneLastFour === phone);
  const rows = (window.historyParticipations || []).filter(item => item.phoneLastFour === phone);
  if (!player || !rows.length) { result.textContent = '未找到该玩家的历史战绩'; return; }
  ...
  result.className = 'player-stats-card';
  result.innerHTML = `<h3>${escapeHTML(player.nickname)} (${escapeHTML(phone)})</h3><div class="ps-grid"><div class="ps-item"><div class="ps-value">${rows.length}</div><div class="ps-label">参赛次数</div></div><div class="ps-item"><div class="ps-value">${wins}/${top3}</div><div class="ps-label">冠军 / 前三</div></div><div class="ps-item"><div class="ps-value">${formatNumber(chips)}</div><div class="ps-label">总奖励筹码</div></div><div class="ps-item"><div class="ps-value">${averageRank.toFixed(1)}</div><div class="ps-label">平均排名</div></div><div class="ps-item"><div class="ps-value">${mushrooms}</div><div class="ps-label">蘑菇使用</div></div></div><h4 class="recent-games-title">最近比赛</h4><ul class="tournament-list">${rows.sort((a, b) => String(tournaments.get(b.tournamentId)?.date || '').localeCompare(String(tournaments.get(a.tournamentId)?.date || ''))).map(item => { const tournament = tournaments.get(item.tournamentId); return `<li class="tournament-item" onclick="showTournamentDetail('${item.tournamentId}')"><div class="t-header"><span class="t-name">${escapeHTML(tournament?.name || item.tournamentId)}</span><span class="t-date">${tournament?.date ? new Date(tournament.date).toLocaleDateString('zh-CN') : '—'}</span></div><div class="t-stats">第 ${item.finalRank} 名 · 奖励 ${formatNumber(item.prizeChips)} · 蘑菇 ${item.mushroomsUsed || 0}</div></li>`; }).join('')}</ul>`;
}
