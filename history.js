...
function renderHistoryTab(tab) {
  ...
  if (!content) return;
  if (tab === 'leaderboard') {
    content.innerHTML = `<div class="leaderboard-tabs"><button class="lb-tab active" data-board="points">总积分榜</button><button class="lb-tab" data-board="wins">胜率榜</button><button class="lb-tab" data-board="games">常客榜</button><button class="lb-tab" data-board="top3">前三率榜</button></div><div id="leaderboard-content"></div>`;
...
function renderLeaderboard(board) {
  ...
  const data = players.map(player => {
    ...
    return { player, games: rows.length, wins, top3, points: rows.reduce((sum, row) => sum + (row.prizePoints || 0), 0) };
  }).filter(item => item.games).sort((a, b) => board === 'points' ? b.points - a.points : board === 'wins' ? (b.wins / b.games) - (a.wins / a.games) : board === 'games' ? b.games - a.games : (b.top3 / b.games) - (a.top3 / a.games));
  const content = document.getElementById('leaderboard-content');
  if (!content) return;
  content.innerHTML = `<ul class="leaderboard-list">${data.map((item, index) => `<li class="lb-item"><span class="lb-rank rank-${index + 1}">${index + 1}</span><div class="lb-info"><div class="lb-name">${escapeHTML(item.player.nickname)} (${escapeHTML(item.player.phoneLastFour)})</div><div class="lb-stats">${item.games}场 · ${item.wins}冠 · 前三${item.top3}次</div></div><span class="lb-value">${board === 'points' ? formatNumber(item.points) : board === 'wins' ? `${Math.round(item.wins / item.games * 100)}%` : board === 'games' ? item.games : `${Math.round(item.top3 / item.games * 100)}%`}</span></li>`).join('') || '<li class="empty-state">暂无数据</li>'}</ul>`;
}
...
async function searchPlayerHistory(event) {
  ...
  result.innerHTML = matches.map(player => {
    ...
    const points = rows.reduce((sum, item) => sum + (item.prizePoints || 0), 0);
    ...
    return `<div class="player-stats-card"><h3>${escapeHTML(player.nickname)} (${escapeHTML(player.phoneLastFour)})</h3><div class="ps-grid"><div class="ps-item"><div class="ps-value">${rows.length}</div><div class="ps-label">参赛次数</div></div><div class="ps-item"><div class="ps-value">${wins}/${top3}</div><div class="ps-label">冠军 / 前三</div></div><div class="ps-item"><div class="ps-value">${formatNumber(points)}</div><div class="ps-label">总积分</div></div><div class="ps-item"><div class="ps-value">${averageRank.toFixed(1)}</div><div class="ps-label">平均排名</div></div><div class="ps-item"><div class="ps-value">${mushrooms}</div><div class="ps-label">蘑菇使用</div></div></div><h4 class="recent-games-title">最近比赛</h4><ul class="tournament-list">${rows.sort((a, b) => String(tournaments.get(b.tournamentId)?.date || '').localeCompare(String(tournaments.get(a.tournamentId)?.date || ''))).map(item => { const tournament = tournaments.get(item.tournamentId); return `<li class="tournament-item" onclick="showTournamentDetail('${item.tournamentId}')"><div class="t-header"><span class="t-name">${escapeHTML(tournament?.name || item.tournamentId)}</span><span class="t-date">${tournament?.date ? new Date(tournament.date).toLocaleDateString('zh-CN') : '—'}</span></div><div class="t-stats">第 ${item.finalRank} 名 · 积分 ${formatNumber(item.prizePoints)} · 蘑菇 ${item.mushroomsUsed || 0}</div></li>`; }).join('')}</ul></div>`;
  }).join('');
}
